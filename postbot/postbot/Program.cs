...
namespace postbot
{
   internal class Program
   {
      ...
      static void Main(string[] args)
      {
         ...
         Timer time_2 = new Timer(callbackLoop, null, 0, 22222);
         Console.ReadLine();
      }
      ...
      private static void CheckPost(object state)
      {
         ...
         for (int i = 0; i < posts.Count; i++) {
            ...
            if (posts[i].date.Split(' ')[0] == now.ToString().Trim('{').Trim('}').Split(' ')[0] && posts[i].date.Split(' ')[1].Split(':')[0] == hour && posts[i].date.Split(' ')[1].Split(':')[1] == minute) {
               SendPost("PostOutNow♥" + posts[i].id, posts[i].id_chanel, "post");
            }
         }
      }

      public static bool isInRange(string tstart, string tend)
      {
         TimeSpan now = DateTime.Now.TimeOfDay;
         TimeSpan start = new TimeSpan(Convert.ToInt32(tstart.Split(':')[0]), Convert.ToInt32(tstart.Split(':')[1]), 0);
         TimeSpan end = new TimeSpan(Convert.ToInt32(tend.Split(':')[0]), Convert.ToInt32(tend.Split(':')[1]), 0);
         ...
         try {
            ...
            for (int i = 0; i < loops.Count; i++) {
               if (loops[i].process == "start") {
                  if (loops[i].time_off != "" && loops[i].time_off != null) check = isInRange(loops[i].time_off.Split('-')[0], loops[i].time_off.Split('-')[1]);
                  if (check == false) {
            ...
            try {
               ...
               for (int i = 0; i < loops.Count; i++) {
                  if (loops[i].id_user == message.Chat.Id.ToString()) {
                     try {
                        int time = Convert.ToInt32(loops[i].time_interval);
                        Array.Resize(ref loopArray, loopArray.Length + 1);
                        loopArray[loopArray.Length - 1] = loops[i].name + "‼" + loops[i].id;
                     } catch {
                        string[] post = loops[i].posts.Split('♥');
                        for (int j = 0; j < post.Length; j++) if (post[j] != "") request += "delete from `Post` where id = " + post[j] + ";\n";
                        request += "delete from `Loop` where id = " + loops[i].id + ";\n";
                     }
                  }
               }
               ...
               if (request != string.Empty) ConnectDB.Query(request);
            } catch { }
         }
      }
      ...
      private static async void InlineButtonOperation(object sc, CallbackQueryEventArgs ev)
      {
         ...
         if (data == "AddChanel") {
                              ...
                              for (int j = 0; j < updates.Length; j++) {
                                 try {
                                    if (updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Document || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Audio || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Photo || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Video) {
                                       postId = posts.Find(x => x.id_user == message.Chat.Id.ToString() && x.id_chanel == data.Split('♥')[1].Split('‼')[1] && x.status == "progress");
                           ...
                           if (text.Contains('.') && text.Contains(':') && text.Contains(" ")) {
                              if (text.Split(' ').Length == 2 && text.Split(':').Length == 2 && text.Split('.').Length == 3) {
                                 try {
                                    ...
                                    ConnectDB.Query("update `Post` set date = '" + time.ToString().Trim('{').Trim('}') + "' where id = " + data.Split('♥')[1] + ";");
            ...
            try {
               ...
               for (int i = 0; i < posts.Length; i++) if (posts[i] != "" && posts[i] != null && posts[i] != string.Empty) {
                     request += "delete from `Post` where id = " + posts[i] + ";\n";
                  }
            ...
            try {
               ...
               if (loop.process == "start") {
                  var keyborad = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Остановить цикл", "StopLoop♥" + loop.id) }, new[] { InlineKeyboardButton.WithCallbackData("Не рабочее время", "NotWorkingTime♥" + loop.id) }, new[] { InlineKeyboardButton.WithCallbackData("Удалить цикл", "DelLoop♥" + loop.id) } });
                  ...
                  var keyborad = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Запустить цикл", "StartLoop♥" + loop.id) }, new[] { InlineKeyboardButton.WithCallbackData("Не рабочее время", "NotWorkingTime♥" + loop.id) }, new[] { InlineKeyboardButton.WithCallbackData("Удалить цикл", "DelLoop♥" + loop.id) } });
                  await client.EditMessageTextAsync(message.Chat.Id, message.MessageId, "Цикл " + loop.name, replyMarkup: keyborad);
               }
            } catch { }
         }
         else if (data.Contains("NotWorkingTime♥")) {
            try {
               await client.EditMessageTextAsync(message.Chat.Id, message.MessageId, "Введите временной интервал, в который цикл не будет работать (Пример: 00:00 - 12:00)");
               int s = 0;
               while (true) {
                  s++;
                  var updates = await client.GetUpdatesAsync(s);
                  for (int i = 0; i < updates.Length; i++) {
                     if (updates[i].Message.From.Id == message.Chat.Id) {
                        if (updates[i].Message.Type == Telegram.Bot.Types.Enums.MessageType.Text) {
                           if (updates[i].Message.Text == "/start" || updates[i].Message.Text == "/chanels" || updates[i].Message.Text == "/loops") { return; }
                           string time = updates[i].Message.Text;
                           if (time.Contains("-") && time.Split(':').Length == 3) {
                              if (time.Split(':')[0].Length == 2 && time.Split(':')[2].Length == 2) {
                                 time = time.Replace(" ", string.Empty);
                                 if (time.Split('-').Length == 2) {
                                    ConnectDB.Query("update `Loop` set time_off = '" + time + "' where id = " + data.Split('♥')[1] + ";");
                                    await client.SendTextMessageAsync(message.Chat.Id, "Для цикла установлено не рабочее время с " + time.Split('-')[0] + " по " + time.Split('-')[1]);
                                    return;
                                 }
                              }
                           }
                           await client.SendTextMessageAsync(message.Chat.Id, "Вы ввели неверный формат периода, введите его еще раз (Пример: 00:00 - 12:00)");
                        }
                     }
                  }
               }
                     ...
                     for (int i = 0; i < updates.Length; i++) {
                        if (updates[i].Message.From.Id == message.Chat.Id) {
                           if (updates[i].Message.Type == Telegram.Bot.Types.Enums.MessageType.Text) {
                              ...
                              if (name.Length > 10) name = name.Remove(10, name.Length - 10) + "...";
                              ...
                              while (true) {
                                 ...
                                 for (int j = 0; j < updates.Length; j++) {
                                    try {
                                       if (updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Document || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Audio || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Photo || updates[j].Message.Type == Telegram.Bot.Types.Enums.MessageType.Video) {
                                          ConnectDB.LoadPost(posts);
                                          postId = posts.Find(x => x.id_user == message.Chat.Id.ToString() && x.status == "progress").id;
                                 ...
                                 if (status == "preview") {
                                    var keyboradPreview = new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Сохранить пост в цикле", "SavePostLoop♥" + postId + "‼" + data.Split('♥')[1]) }, new[] { InlineKeyboardButton.WithCallbackData("Отменить", "CancelPostLoop♥" + postId + "‼" + data.Split('♥')[1]) } });
            ...
            string request = string.Empty;
            try {
               request += "delete from `Post` where id = " + data.Split('♥')[1].Split('‼')[0] + ";\n";
               ConnectDB.LoadLoop(loops);
               var loop = loops.Find(x => x.id.ToString() == data.Split('♥')[1].Split('‼')[1]).posts;
               loop = loop.Replace(data.Split('♥')[1].Split('‼')[0], string.Empty).Trim('♥');
               ConnectDB.Query(request);
               CreateLoopProcess(data.Split('♥')[1].Split('‼')[1], message);
            } catch { }
         }
      ...
      private static InlineKeyboardButton[][] GetInlineKeyboardChanels(string[] stringArray)
      {
         ...
         for (int i = 0; i < stringArray.Length + 1; i++) {
            ...
            if (i + 1 != stringArray.Length + 1) {
               string text = stringArray[i].Split('‼')[0];
               if (text.Length > 10) text = text.Remove(10, text.Length - 10) + "...";
               keyboardButtons[0] = new InlineKeyboardButton {
                  Text = text,
                  CallbackData = "MoreChanel♥" + text + "‼" + stringArray[i].Split('‼')[1],
               };
            }
            else {
               keyboardButtons[0] = new InlineKeyboardButton {
      ...
      private static InlineKeyboardButton[][] GetInlineKeyboardChanelsLoop(string[] stringArray, string id)
      {
         ...
         for (int i = 0; i < stringArray.Length + 1; i++) {
            ...
            if (i + 1 != stringArray.Length + 1) {
               ...
               keyboardButtons[0] = new InlineKeyboardButton {
                  Text = text,
                  CallbackData = "AddLoopChanel♥" + text + "‼" + stringArray[i].Split('‼')[1] + "‼" + id,
               };
            }
            else {
               keyboardButtons[0] = new InlineKeyboardButton {
         ...
         for (int i = 0; i < stringArray.Length + 1; i++) {
            ...
            if (i + 1 != stringArray.Length + 1) {
               keyboardButtons[0] = new InlineKeyboardButton {
                  ...
                  CallbackData = "MoreLoop♥" + stringArray[i].Split('‼')[1],
               };
            }
            else {
               keyboardButtons[0] = new InlineKeyboardButton {
